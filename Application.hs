...
mkYesodDispatch "Yiki" resourcesYiki
...
withYiki :: AppConfig -> Logger -> (Application -> IO a) -> IO ()
withYiki conf logger f = do
    s <- static Settings.staticDir
    Settings.withConnectionPool conf $ \p -> do
        runConnectionPool (runMigration migrateAll) p
        let h = Yiki conf logger s p
#ifdef WINDOWS
        toWaiApp h >>= f >> return ()
#else
...
withDevelAppPort =
  ...
  where
    ...
    go f = do
        ...
        withYiki conf logger $ \app -> f (port, debugHandle (logHandle logger) app)
        flushLogger logger
      where
